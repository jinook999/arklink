...
class FileRequest extends FRONT_Controller {
	...
	public function remove(){
		try {
			if(defined("_IS_AJAX")) {
				$this->form_validation->set_rules("folder", print_language("filerequest_folder"), "trim|required|xss_clean");
				$this->form_validation->set_rules("fname", print_language("filerequest_file_name"), "trim|required|xss_clean");
				if(!$this->form_validation->run()){
					$err = $this->form_validation->error_array();
				...
				if(!file_exists($file_path)){
					echo json_encode(array("error" => print_language("no_files_on_the_route")));
					exit;
				}
	...
	public function download() {
		try {
			...
			if(!isset($file)) {
				throw new Exception(print_language("no_file_information_to_download"));
			}
			...
			if(!preg_match('/\/upload\//i', @realpath($download_info['dirname'])) || preg_match('/(\/data\/|\/\_compile\/|\/install\/|\/latest\/|\/lib\/|\/system\/)/i', @realpath($download_info['dirname'])) || preg_match('/\.\.\//i', $file)) {
				throw new Exception(print_language("download_path_cannot_be_downloaded"));
			}

			if(!is_dir($download_info['dirname'])) {
				throw new Exception(print_language("download_path_does_not_exist"));
			}

			if(!file_exists($download_file)) {
				throw new Exception(print_language("no_files_to_download"));
			}
			...
			if(!$is_validate) {
				throw new Exception(print_language("file_can_not_be_downloaded"));
			}
