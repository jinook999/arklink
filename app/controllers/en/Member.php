	...
	public function login() {
		try {
			...
			if ($this->form_validation->run() == true) {
				...
				$set_data["member"][ib_isset($this->_site_language, "eng")] = array(
		...
		try {
			...
			if($this->form_validation->run()) {
				...
				if(isset($result['userid'])) {
					...
					$form = form_open('en/member/find_ok', ['name' => 'frm', 'method' => 'POST'], $data).form_close();
	...
	public function join() {
		try {
			...
			if(isset($_REQUEST['agree']) == false && isset($_REQUEST['agree2']) == false) {
				...
				echo '<script>alert("'.$msg.'");window.location.href = "join_agreement" ;</script>';
			}
			//Matthew End
	...
	public function mypage() {
		try {
			...
			if($this->form_validation->run()){
				...
				if($result) {
					msg(print_language("edit_membership_information"), "/en/member/mypage");
				}
			} else {
				if(validation_errors()) {
					throw new Exception(validation_errors());
