...
class Board extends FRONT_Controller {
	...
	public function board_secret() {
		try {
			...
			$form_attribute = array(
				"action" => "/en/board/board_". $page ."?code=". $this->_board["code"] ."&no=". $no,
				"attribute" => array(
					"method" => "POST",
					"name" => "frm",
				),
				...
				if($this->form_validation->run()){
					...
					if($no) {
						...
						if($this->_board['yn_display_list'] == 'y'){
							// 로그인 시
							if(defined("_IS_LOGIN")) {
								go("/en/board/board_view?code=". $this->_board["code"] ."&no=". $no, "parent");
							} else {
								if($is_secret == "y") {
									go("/en/board/board_list?code=". $this->_board["code"] ."&no=". $no ."&page=view", "parent");
								} else {
									//go("/board/board_view?code=". $this->_board["code"] ."&no=". $no, "parent");
									go("/en/board/board_list?code=". $this->_board["code"], "parent");
								}
							}
						} else {
							msg(print_language('inquiry_is_registered'), "/en/board/board_write?code=". $this->_board["code"], "parent");
						}
					} else {
				...
				if(in_array($mode, array("inquire_answer_modify", "modify", "answer"))) {
                    ...
                    if($mode == "modify") {
                        ...
                        if(!empty($board_view['board_view']['userid'])) {
                            ...
                            $no = $board_view['board_view']['no'];
                            if(empty($password)) {
                                go("/enboard/board_secret?page=write&code=$code&no=$no");
                            }
	...
	public function board_delete() {
		try {
            ...
            if(!empty($board_view['board_view']['userid'])) {
                ...
                $no = $board_view['board_view']['no'];
                if(empty($password)) {
                    go("/en/board/board_secret?page=delete&code=$code&no=$no");
                }
			...
			if($result) {
				...
				msg(print_language("delete_post"), "/en/board/board_list?code=". $this->_board["code"]);
			} else {
