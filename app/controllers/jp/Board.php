...
class Board extends FRONT_Controller {
	...
	public function index() {
		go("board_list?code=". $this->_board["code"]);
	}
		...
		try {
			...
			if($board_view['board_view']['is_secret'] == 'y'){
				if(!empty($board_view['board_view']['userid'])) {
                    ...
                    if(empty($password)) {
                        go("board_secret?code=$code&no=$no&page=view");
                    }
					...
					if($no) {
						...
						if($this->_board['yn_display_list'] == 'y'){
							...
							if(defined("_IS_LOGIN")) {
								...
								if($is_secret == "y") {
									go("board_list?code=". $this->_board["code"] ."&no=". $no ."&page=view", "parent");
								} else {
									//go("/board/board_view?code=". $this->_board["code"] ."&no=". $no, "parent");
									go("board_list?code=". $this->_board["code"], "parent");
								}
							}
						} else {
							msg(print_language('inquiry_is_registered'), "board_write?code=". $this->_board["code"], "parent");
						}
					} else {
				...
				if(in_array($mode, array("inquire_answer_modify", "modify", "answer"))) {
                    ...
                    if($mode == "modify") {
                        ...
                        if(!empty($board_view['board_view']['userid'])) {
                            ...
                            $no = $board_view['board_view']['no'];
                            if(empty($password)) {
                                go("board_secret?page=write&code=$code&no=$no");
                            }
	...
	public function board_delete() {
		try {
            ...
            if(!empty($board_view['board_view']['userid'])) {
                ...
                $no = $board_view['board_view']['no'];
                if(empty($password)) {
                    go("board_secret?page=delete&code=$code&no=$no");
                }
			...
			if($result) {
				...
				msg(print_language("delete_post"), "board_list?code=". $this->_board["code"]);
			} else {
