...
class Member extends FRONT_Controller {
		...
		try {
			...
			if($this->form_validation->run()) {
				...
				if(isset($result['userid'])) {
					...
					$form = form_open('cn/member/find_ok', ['name' => 'frm', 'method' => 'POST'], $data).form_close();
	...
	public function join() {
		try {
			...
			if(isset($_REQUEST['agree']) == false && isset($_REQUEST['agree2']) == false) {
				...
				echo '<script>alert("'.$msg.'");window.location.href = "/cn/member/join_agreement" ;</script>';
			}
			//Matthew End
	...
	public function mypage() {
		try {
			if(!defined("_IS_LOGIN")) {
				msg(print_language("go_to_the_login_page"), "login?return_url=". urlencode(current_full_url()));
			}
			$arr_ex = array("ex1", "ex2", "ex3", "ex4", "ex5", "ex6", "ex7", "ex8", "ex9", "ex10",
							"ex11", "ex12", "ex13", "ex14", "ex15", "ex16", "ex17", "ex18", "ex19", "ex20"); // 관리자 커스텀 필드
			foreach($this->input->post(null, true) as $key => $val){
			...
			if($this->form_validation->run()){
				...
				if($result) {
					msg(print_language("edit_membership_information"), "/cn/member/mypage");
				}
			} else {
				if(validation_errors()) {
					throw new Exception(validation_errors());
		...
		try {
			...
			if($this->form_validation->run()){
				...
				if($get_data["yn_password"] != "y") {
					...
					$result = $this->Front_Member_model->member_register("modify", $set_data);
					if($result) {
						msg(print_language("change_password"), "change_pw");
					}
				}
				throw new Exception(print_language("no_member_information_found"));
			} else {
		...
		try {
			...
			if($this->form_validation->run()) {
				...
				if($get_data["yn_password"] != "y") {
					...
					if($result) {
						msg(print_language("member_withdrawal_processing"), "logout");
					}
				}
				throw new Exception(print_language("no_member_information_found"));
			} else {
...
//exit;
					...
					if($result) {
						...
						$this->session->set_userdata(array("member" => $this->_member));
						msg(print_language("change_password"), base_url().'cn');
					}
				}
