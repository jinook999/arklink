...
class Board extends FRONT_Controller {
	...
	public function board_list() {
		try {
			...
			$config = array(
				...
				"per_page" => $limit,
				"first_url" => "?code=". $this->_board["code"]."&search_type=". $search_type ."&search=". $search."&category=".$category,
				"suffix" => "&code=". $this->_board["code"]."&search_type=". $search_type ."&search=". $search."&category=".$category,
			);
	...
	public function board_view() {
		try {
            ...
            $navi_like = [];
            if($get['search_type'] && $get['search']) {
                $navi_like = [$get['search_type'] => $get['search'].'|both'];
            }
            $navi['next'] = $this->dm->get('da_board_'.$this->_board['code'], [], ['no > ' => $get['no'], 'fixed' => 0], $navi_like, [], ['no' => 'ASC'])[0];
            $navi['prev'] = $this->dm->get('da_board_'.$this->_board['code'], [], ['no < ' => $get['no'], 'fixed' => 0], $navi_like, [], ['no' => 'DESC'])[0];

			$this->template_->assign('navi', $navi);
			...
			try {
				...
				*/
				...
				if(in_array($mode, array("write", "modify", "answer"))) {
					...
					if($this->_board["code"] == "diagnosis") {
						$this->form_validation->set_rules("gender", "성별", "trim|required|xss_clean");
					}
