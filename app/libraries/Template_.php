...
class Template_
{
	...
	function __construct() {
		$template_path =  str_replace(DIRECTORY_SEPARATOR.'system'.DIRECTORY_SEPARATOR, '', BASEPATH);
		$this->compile_dir = $template_path .'/_compile';
		$this->template_dir = $template_path .'/data/skin';
		//$this->prefilter = 'adjustPath|defaultScript';
		$this->prefilter = 'adjustPath & css,jpg,jpeg,png,gif,swf';
	...
	function clearCache()
	{
		...
		if (!func_num_args())
		{
			$this->_rmdir($this->cache_dir, $safe_mode);
		}
		else
		{
			foreach (func_get_args() as $dir)
			{
				$this->_rmdir($this->cache_dir.'/%clear/'.$dir, $safe_mode);
			}
		}
	}
