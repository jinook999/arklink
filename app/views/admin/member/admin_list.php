...
<div id="contents">
	<div class="main_tit">
		...
		<div class="btn_right btn_num2">
			...
			<a href="admin_reg" class="btn point new_plus">+ 관리자 등록</a>
		</div><!--btn_right-->
	</div>
		...
		<fieldset>
			...
			<select name="search_level">
                ...
                <? foreach($member_grade_list as $value) :?>
                    <option value="<?=$value["level"]?>" <?=set_select("search_level", $value["level"])?>><?=$value['level'].' - '.$value["gradenm"]?></option>
                <? endforeach; ?>
			</select>
				...
				<tbody id='divList'>
					<? foreach($member_list as $key => $value) : ?>
						<tr>
							...
							<td><?=$value["userid"]?></td>
							<td><?=$value["name"]?></td>
							<td><a href="admin_grade_reg?level=<?=$value['level']?>"><?=$value['level']?></a></td>
							<td><a href="admin_grade_reg?level=<?=$value['level']?>"><?=$member_grade_text[$value["level"]]?></a></td>
							<td><?=$value["email"]?></td>
							<td><?=$value["mobile"]?></td>
