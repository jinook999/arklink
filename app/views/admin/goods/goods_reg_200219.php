...
<script>
		...
		$("form[name='frm']").validate({
			rules : {
				...
				<? for($idx = 1; $idx<=20; $idx++){ ?>
					<? if(!empty($goodsField["multi"]["ex".$idx])): ?>
						<? foreach($this->_site_language["set_language"] as $key => $value): ?>
	...
	function getColumnValue(name) {
		...
		if(fieldOption[name] && fieldOption[name]["type"]) {
			if(fieldOption[name]["type"] == "file") {
				value = $("input[name='"+ name +"_fname']").val();
			} else if(fieldOption[name]["type"] == "editor") {
				value = getSmartEditor(name);
			} else if(fieldOption[name]["type"] == "checkbox" || fieldOption[name]["type"] == "radio") {
				value = $("[name='"+ name +"']:checked").val();
			} else if(fieldOption[name]["type"] == "select") {
...
<?=form_open("", array("name" => "frm", "id" => "frm"))?>
	...
	<div class="main_tit">
		...
		<div class="location" style="margin-left:20px;">
			<h4><button type="button" class="info_bullet">URL</button> <?=base_url()?>goods/goods_view?no=<?=$this->input->get('no', true)?> <a class="btn-sm" href="<?=base_url()?>goods/goods_view?no=<?=$this->input->get('no', true)?>" target="_blank">바로가기</a></h4>
					...
					<? foreach($goodsField["name"][$site_language] as $key => $value) : ?>
						<? if($goodsField["use"][$this->_site_language["default"]][$key] == "checked") : ?>
							...
							<tr>
								...
								<td>
									<? if($goodsField["option"][$key]) : ?>
										<? if(in_array($goodsField["option"][$key]["type"], array("checkbox", "radio"))) :?>
											<? foreach($goodsField["option"][$key]["item"][$site_language] as $subKey => $subValue) : ?>
												<input type="<?=$goodsField["option"][$key]["type"]?>" id="<?=$key?>-<?=$subKey?>" name="<?=$key?>" value="<?=$subKey?>" <? if($goods_view[$key] == $subKey ) : ?>checked<? endif ?>>
												<label for="<?=$key?>-<?=$subKey?>"><?=$subValue?></label>
											<? endforeach; ?>
										<? elseif($goodsField["option"][$key]["type"] == "select") : ?>
											<select name="<?=$key?>">
												<? foreach($goodsField["option"][$key]["item"][$site_language] as $subKey => $subValue) : ?>
													<option value="<?=$subKey?>" <? if($goods_view[$key] == $subKey ) : ?>selected<? endif ?>><?=$subValue?></option>
												<? endforeach; ?>
											<select>
										<? elseif($goodsField["option"][$key]["type"] == "editor") : ?>
										...
										<? elseif($goodsField["option"][$key]["type"] == "file" && $key != "detail_img") : ?>
											<input type="file" name="<?=$key?>"/>
											<input type="hidden" name="<?=$key?>_oname" />
											<input type="hidden" name="<?=$key?>_fname" value="<?=$goods_view[$key]?>" />
											<? if($goodsField["option"][$key]["type"] == "file") : ?>
												<input type="hidden" name="<?=$key?>_type" value="<?=$goodsField["option"][$key]["file_type"]?>" />
											<? else : ?>
											...
											<?php endif?>
											<? if($goodsField["option"][$key]["file_type"] == "image") : ?>
											<!--이미지 리사이즈용 width, height 추가-->

											<?php if(empty($goodsField['option'][$key]['width']) && empty($goodsField['option'][$key]['height'])) : ?>
											...
											<?php else : ?>
												<p>
												* 권장 사이즈 값으로 리사이징되어 등록됩니다. ( 권장사이즈 : 가로 <?=$goodsField["option"][$key]["width"]?>px X 세로 <?=$goodsField["option"][$key]["height"]?>px )
												</p>
												<? if(!empty($max_file_uploads)){ ?>
												<p>* 최대 업로드 가능 이미지 용량 : <?=$max_file_uploads?>mb</p>
												<? } ?>
												<input type="hidden" name="<?=$key?>_width" value="<?=$goodsField["option"][$key]["width"]?>">
												<input type="hidden" name="<?=$key?>_height" value="<?=$goodsField["option"][$key]["height"]?>">
											<?php endIf ?>
							...
							<? if($key == "detail_img") :  ?>
								<? if(!empty($goods_view["detail_img"])){ ?>
									...
									<? foreach($goods_view["detail_img"] as $fkey => $fval){ ?>
										<? if(!empty($fval)){ ?>
										<tr class = "detail_tr">
											<th class="ta_left" class = "detail_th">상세이미지 <?=($fkey + 1)?></th>
											<td>
				...
				<div class="table_write">
					<table width="100%" cellpadding="0" cellspacing="0" border="0">
						...
						<tbody id='divList'>
							...
							?>
							<? foreach($goodsField["name"][$site_language] as $key => $value) : ?>
								<? if(in_array($key, $exceptKey)) continue; ?>
								<? if($goodsField["use"][$this->_site_language["default"]][$key] == "checked") : ?>
									<? if(array_key_exists($key,$goodsField["multi"]) && $this->_site_language["multilingual"]) :  ?>
									<!--multi-->
									<!--foreach Start-->
										<? foreach($this->_site_language["set_language"] as $fkey => $fval){ ?>
										<tr>
											<th class="ta_left"><? if($goodsField["require"][$this->_site_language["default"]][$key] == "checked") : echo "<em>*</em> "; endif; ?><?=$value?>(<?=$fval?>)</th>
											<td>
												<? if($goodsField["option"][$key]) : ?>
													<? if(in_array($goodsField["option"][$key]["type"], array("checkbox", "radio"))) :?>
														<? foreach($goodsField["option"][$key]["item"][$site_language] as $subKey => $subValue) : ?>
													...
													<? elseif($goodsField["option"][$key]["type"] == "select") : ?>
														...
														<select>
													<? elseif($goodsField["option"][$key]["type"] == "editor") : ?>
														<div class="editor-box"><textarea name="<?=$key?>_<?=$fkey?>" id="<?=$key?>_<?=$fkey?>" class="editor" style="width: 100%;height: 400px; "><?=$goods_view["multi"][$key][$fkey]?></textarea></div>
														<script>attachSmartEditor("<?=$key?>_<?=$fkey?>", "goods");</script>
													<? elseif($goodsField["option"][$key]["type"] == "file") : ?>
														<input type="file" name="<?=$key?>"/>
														<input type="hidden" name="<?=$key?>_oname" />
														<input type="hidden" name="<?=$key?>_fname" value="<?=$goods_view[$key]?>" />
														<? if($goodsField["option"][$key]["type"] == "file") : ?>
													...
													<? endif; ?>
												<? else : ?>
													<input type="text" name="<?=$key?>_<?=$fkey?>" value="<?=$goods_view[$key]?>" />
												<? endif; ?>
											</td>
										</tr>
										<? } ?>
									<!--foreach End-->
									<? elseif($key != "detail_img" || $key != "img1" || $key != "img2") : ?>
									<!--normal-->
									<tr>
										<th class="ta_left"><? if($goodsField["require"][$this->_site_language["default"]][$key] == "checked") : echo "<em>*</em> "; endif; ?><?=$value?></th>
										<td>
											<? if($goodsField["option"][$key]) : ?>
												...
												<? elseif($goodsField["option"][$key]["type"] == "file" && $key != "detail_img") : ?>
													<input type="file" name="<?=$key?>"/>
													<input type="hidden" name="<?=$key?>_oname" <? if($key == "upload_fname"): ?>value="<?=$goods_view["upload_oname"]?>"<? endif; ?>/>
													<input type="hidden" name="<?=$key?>_fname" value="<?=$goods_view[$key]?>" />
													<? if($goodsField["option"][$key]["type"] == "file") : ?>
														<input type="hidden" name="<?=$key?>_type" value="<?=$goodsField["option"][$key]["file_type"]?>" />
													<? else : ?>
